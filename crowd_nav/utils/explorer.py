...
class Explorer(object):
    ...
    def run_k_episodes(self, k, phase, update_memory=False, imitation_learning=False, episode=None, epoch=None,
                       print_failure=False, memory_name='rl_experience', memory_interval=1000):
        ...
        for i in range(k):
            ...
            if update_memory:
                ...
                num_episode = i + 1 + self.memory.num_episode_resume if imitation_learning else episode + 1
                if num_episode % memory_interval == 0:
                    self.memory.save(memory_name, num_episode)
    ...
    def update_memory(self, states, actions, rewards, imitation_learning=False):
        ...
        for i, state in enumerate(states):
            ...
            if imitation_learning:
                ...
                if i == len(states) - 1:
                    next_state = (torch.zeros_like(state[0]), torch.zeros_like(state[1]))
                else:
