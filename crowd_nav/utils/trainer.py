...
class MPRLTrainer(object):
    ...
    def optimize_epoch(self, num_epochs):
        ...
        for epoch in range(num_epochs):
            ...
            for data in self.data_loader:
                ...
                terminal_mask = (next_human_states[..., -1] != 0)
                ...
                if self.state_predictor.trainable:
                    ...
                    if update_state_predictor:
                        ...
                        loss = (next_human_states_est - next_human_states).pow(2).mean(-1)
                        loss = torch.where(terminal_mask, loss, torch.zeros_like(loss))
                        loss = loss.sum() / terminal_mask.sum()
                        loss.backward()
    ...
    def optimize_batch(self, num_batches, episode):
        ...
        for data in self.data_loader:
            ...
            terminal_mask = (next_human_states[..., -1] != 0)
            value_mask = (torch.sum(terminal_mask, dim=-1, keepdim=True) != 0)
            # optimize value estimator
            self.v_optimizer.zero_grad()
            outputs = self.value_estimator((robot_states, human_states))
            ...
            target_values = torch.where(value_mask, target_values, rewards)

            # values = values.to(self.device)
            loss = self.criterion(outputs, target_values.detach())
            loss.backward()
            self.v_optimizer.step()
