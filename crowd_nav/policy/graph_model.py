...
class RGL(nn.Module):
    ...
    def compute_similarity_matrix(self, X,mask_horizontal, mask_vertical):
        if self.similarity_function == 'embedded_gaussian':
            A = torch.matmul(torch.matmul(X, self.w_a), X.permute(0, 2, 1))
            A[~mask_horizontal] = float('-inf')
            normalized_A = softmax(A, dim=2)
            normalized_A = torch.where(mask_vertical, normalized_A, torch.zeros_like(normalized_A))
    ...
    def forward(self, state):
        ...
        num_peds = (human_states[..., -1] != 0).sum(-1)
        mask_vertical  = (human_states[..., -1] != 0).unsqueeze(-1).repeat(1, 1, shape[1])
        mask_horizontal = sequence_mask(num_peds, shape[1]).unsqueeze(-2).repeat(1, shape[1], 1)
        mask_vertical = self.pad(mask_vertical.unsqueeze(0).float()).squeeze(0).bool()
        mask_horizontal = self.pad(mask_horizontal.unsqueeze(0).float()).squeeze(0).bool()

        # compute matrix A
        if not self.layerwise_graph:
            normalized_A = self.compute_similarity_matrix(X, mask_horizontal, mask_vertical)
