...
class ModelPredictiveRL(Policy):
    def __init__(self):
        ...
        self.traj = None
        self.action_space_torch = None
        self.normalized_gamma = None
    ...
    def configure(self, config):
        ...
        if self.linear_state_predictor:
            ...
            if self.share_graph_model:
                ...
                graph_model1 = RGL(config, self.value_robot_state_dim, self.human_state_dim)
    ...
    def set_time_step(self, time_step):
        ...
        self.normalized_gamma = self.get_normalized_gamma()
        ...
        for j, speed in enumerate(speeds):
            ...
            for i, rotation in enumerate(rotations):
                ...
                if holonomic:
                    ...
                    action_space_array.append([speed * np.cos(rotation), speed * np.sin(rotation)])
                else:
                    action_space.append(ActionRot(speed, rotation))
                    action_space_array.append([speed, rotation])

        self.speeds = speeds
        self.rotations = rotations
        self.action_space = action_space
        self.action_space_torch = torch.tensor(np.array(action_space_array), dtype=torch.float64).unsqueeze(1)
    ...
    def predict(self, state):
        ...
        if self.phase == 'train' and probability < self.epsilon:
            ...
            state_tensor = state.to_tensor(add_batch_size=True, device=self.device)
            index = self.search_action(state, state_tensor)
            max_action = self.action_space[index]
    ...
    def predict_state(self, state, action_space):
        batch_state = state[0].shape[0]
        batch_action = action_space.shape[0]
        next_human_state_est = self.state_predictor(state, None)[1]
        next_human_state_est = next_human_state_est.repeat(batch_action, 1, 1)
        robot_state = state[0].repeat(batch_action, 1, 1)
        actions = action_space.repeat(1, 1, batch_state).reshape(batch_state * batch_action, 1, 2)
        next_robot_state_est = self.state_predictor.compute_robot_next_state(robot_state, actions)
        next_state = (next_robot_state_est, next_human_state_est)
        next_state_value = self.value_estimator(next_state)
        return next_state, next_state_value

    def search_action(self, original_state, state):
        values = []
        next_state, next_state_value = self.predict_state(state, self.action_space_torch)
        for i, action in enumerate(self.action_space):
            reward_est = self.estimate_reward(original_state, action)
            value = reward_est + self.normalized_gamma * next_state_value[i].cpu().detach().numpy()
            values.append(value)
    ...
    def estimate_reward(self, state, action):
        ...
        for i, human in enumerate(human_states):
            ...
            if self.kinematics == 'holonomic':
                ...
                vx = human.vx - action.v * np.cos(action.r * self.time_step + robot_state.theta)
                vy = human.vy - action.v * np.sin(action.r * self.time_step + robot_state.theta)
        ...
        if self.kinematics == 'holonomic':
            ...
            theta = robot_state.theta + action.r * self.time_step
