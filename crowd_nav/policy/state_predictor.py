...
class StatePredictor(nn.Module):
    ...
    def compute_next_state(self, robot_state, action):
        ...
        return next_state.unsqueeze(0).unsqueeze(0).cuda()

    def compute_robot_next_state(self, robot_state, action):
        next_state = robot_state.clone().cpu()
        next_state[..., 8] = next_state[..., 8] + (action[..., 1] * self.time_step).float()
        next_state[..., 0] = next_state[..., 0] + np.cos(next_state[..., 8]) * (action[..., 0] * self.time_step).float()
        next_state[..., 1] = next_state[..., 1] + np.sin(next_state[..., 8]) * (action[..., 0] * self.time_step).float()
        next_state[..., 2] = np.cos(next_state[..., 8]) * action[..., 0].float()
        next_state[..., 3] = np.sin(next_state[..., 8]) * action[..., 0].float()
        return next_state.cuda()
