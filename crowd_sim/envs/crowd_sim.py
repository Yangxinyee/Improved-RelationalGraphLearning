...
class CrowdSim(gym.Env):
    ...
    def generate_robot_pose(self):
        px = 0
        py = -self.circle_radius
        theta = np.random.uniform(0, np.pi)
        gx = np.random.uniform(-self.square_width / 3.0, self.square_width / 3.0)
        gy = np.random.uniform( self.square_width / 3.0, self.square_width / 2.0)
        return px, py, theta, gx, gy
    ...
    def reset(self, phase='test', test_case=None):
        ...
        px, py, theta, gx, gy = self.generate_robot_pose()
    ...
    def step(self, action, update=True):
        ...
        for i, human in enumerate(self.humans):
            ...
            if self.robot.kinematics == 'holonomic':
                ...
                vx = human.vx - action.v * np.cos(action.r * self.time_step + self.robot.theta)
                vy = human.vy - action.v * np.sin(action.r * self.time_step + self.robot.theta)
