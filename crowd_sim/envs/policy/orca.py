...
def get_nonholonomic_velocity(action, theta, time_step):
    v = np.sqrt(action.vx ** 2 + action.vy ** 2)
    relative_theta = np.arctan2(action.vy, action.vx) - theta
    if relative_theta < -np.pi:
        relative_theta += 2 * np.pi
    elif relative_theta > np.pi:
        relative_theta -= 2 * np.pi
    r = relative_theta / time_step
...
class ORCA(Policy):
    ...
    def predict(self, state):
        ...
        if self.kinematics != 'holonomic':
            action = get_nonholonomic_velocity(action, robot_state.theta, self.time_step)
        return action
